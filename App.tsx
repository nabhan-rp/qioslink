...
// --- LANDING PAGE COMPONENT (PRESERVED V3.8 DESIGN) ---
...
export default function App() {
  ...
  const handleManualVerifyUser = async (targetUserId: string) => {
      if(!confirm("Are you sure you want to verify this user manually?")) return;
      
      setApiLoading(true);
      if(IS_DEMO_MODE) {
          const updatedUsers = users.map(u => u.id === targetUserId ? {...u, isVerified: true} : u);
          setUsers(updatedUsers);
          alert("User Verified (Demo)");
      } else {
          try {
              const res = await fetch(`${API_BASE}/manage_users.php?action=verify`, {
  ...
  const handleUpdateAccount = () => {
      ...
      setCurrentUser(updatedUser);
      sessionStorage.setItem('qios_user', JSON.stringify(updatedUser));
      alert("Account Updated Successfully (Demo/Logic Placeholder)");
  ...
  const handleGenerateQR = async () => { 
      ...
      setApiLoading(true); setGeneratedQR(null); setGeneratedLink(null); 
      if (IS_DEMO_MODE) { 
          const qr = generateDynamicQR(config.qrisString, Number(tempAmount)); 
          const token = Math.random().toString(36).substring(7); 
          const link = `${window.location.origin}/?pay=${token}`; 
          setTimeout(() => { 
              setGeneratedQR(qr); setGeneratedLink(link); 
              const newTrx: Transaction = { id: `TRX-${Date.now()}`, merchantId: currentUser?.id || '0', amount: Number(tempAmount), description: tempDesc, status: 'pending', createdAt: new Date().toISOString(), qrString: qr, paymentUrl: link }; 
              setTransactions([newTrx, ...transactions]); 
              setApiLoading(false); 
          }, 800); 
      } else { 
          try { 
              const res = await fetch(`${API_BASE}/create_payment.php`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ merchant_id: currentUser?.id, amount: Number(tempAmount), description: tempDesc, expiry_minutes: expiryMinutes ? parseInt(expiryMinutes) : 0, single_use: singleUse, api_key: config.appSecretKey }) }); 
              const data = await res.json(); 
              if (data.success) { 
                  setGeneratedQR(data.qr_string); 
                  setGeneratedLink(data.payment_url); 
                  const newTrx: Transaction = { id: data.trx_id, merchantId: currentUser?.id || '0', amount: Number(tempAmount), description: tempDesc, status: 'pending', createdAt: new Date().toISOString(), qrString: data.qr_string, paymentUrl: data.payment_url }; 
                  setTransactions([newTrx, ...transactions]); 
              } else { alert("Failed: " + (data.message || "Unknown error")); } 
          } catch (e) { alert("Connection Error. Check your API."); } finally { setApiLoading(false); } 
      } 
  };
  
  const handleRevokeLink = async (trx: Transaction) => { 
      if (currentUser?.isVerified === false) return alert("Unverified users cannot modify links.");
      if (!confirm("Are you sure you want to cancel this payment link? Users won't be able to pay anymore.")) return; 
      if (IS_DEMO_MODE) { 
          const updated = transactions.map(t => t.id === trx.id ? {...t, status: 'cancelled'} : t); /* @ts-ignore */ setTransactions(updated); alert("Link Revoked (Demo)"); 
      } else { 
          try { 
              const res = await fetch(`${API_BASE}/revoke_link.php`, { method: 'POST', body: JSON.stringify({ trx_id: trx.id }) }); 
              const data = await res.json(); 
              if (data.success) { fetchTransactions(currentUser!); alert("Link Revoked Successfully"); } else { alert("Failed: " + data.message); } 
          } catch(e) { alert("Connection Error"); } 
      } 
  };
  ...
  const fetchTransactions = async (user: User) => { 
      if (IS_DEMO_MODE) { 
          ...
          if (savedTx) { setTransactions(JSON.parse(savedTx)); } else { const dummies: Transaction[] = Array(5).fill(0).map((_, i) => ({ id: `TRX-DEMO-${1000+i}`, merchantId: user.id, amount: 10000 + (i * 5000), description: `Demo Transaction ${i+1}`, status: i % 2 === 0 ? 'paid' : 'pending', createdAt: new Date().toISOString(), qrString: user.merchantConfig?.qrisString || '', paymentUrl: window.location.origin + '/?pay=demo' + i })); setTransactions(dummies); } return; 
      } 
      try { const res = await fetch(`${API_BASE}/get_data.php?user_id=${user.id}&role=${user.role}`); const data = await res.json(); if (data.success && data.transactions) setTransactions(data.transactions); } catch (e) { console.error(e); } 
  };
  
  const fetchUsers = async () => { 
      if (IS_DEMO_MODE) { 
          const savedUsers = localStorage.getItem('qios_users'); const parsedSaved = savedUsers ? JSON.parse(savedUsers) : []; const combined = [...MOCK_USERS, ...parsedSaved.filter((u: User) => !MOCK_USERS.find(m => m.id === u.id))]; setUsers(combined); return; 
      } 
      try { const res = await fetch(`${API_BASE}/manage_users.php?action=list`); const data = await res.json(); if (data.success && data.users) setUsers(data.users); } catch (e) { console.error(e); } 
  };
  ...
  const loginSuccess = (user: User, redirect = true) => { 
      ...
      setAccountForm({ username: user.username, email: user.email || '', password: '', newPassword: '', confirmNewPassword: '' }); 
      ...
      if (user.role === 'user') setView('history'); else setView('dashboard'); 
  ...
  const handleRegister = async (e: React.FormEvent) => { 
      ...
      if (regPass !== regConfirmPass) { setRegError('Passwords do not match'); return; }
      setApiLoading(true); 
      if (IS_DEMO_MODE) { 
  ...
  const handleUserManagementSubmit = async (e: React.FormEvent) => { 
      e.preventDefault(); setApiLoading(true); if (!currentUser) return; const payloadConfig = userFormData.role === 'merchant' ? { merchantName: userFormData.merchantName, merchantCode: userFormData.merchantCode, qiospayApiKey: userFormData.apiKey, appSecretKey: 'QIOS_SECRET_' + Math.random().toString(36).substring(7), qrisString: userFormData.qrisString } : null; 
      if (IS_DEMO_MODE) { let updatedUsers = [...users]; if (editingUser) { updatedUsers = updatedUsers.map(u => u.id === editingUser.id ? { ...u, username: userFormData.username, email: userFormData.email, role: userFormData.role, merchantConfig: payloadConfig || undefined } : u); } else { updatedUsers.push({ id: Date.now().toString(), username: userFormData.username, email: userFormData.email, role: userFormData.role, merchantConfig: payloadConfig || undefined, isVerified: true }); } setUsers(updatedUsers); localStorage.setItem('qios_users', JSON.stringify(updatedUsers.filter(u => !MOCK_USERS.find(m => m.id === u.id)))); alert('Saved (Demo)'); } else { try { const res = await fetch(`${API_BASE}/manage_users.php`, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ action: editingUser ? 'update' : 'create', id: editingUser?.id, username: userFormData.username, email: userFormData.email, password: userFormData.password, role: userFormData.role, config: payloadConfig, creator_role: currentUser.role }) }); const data = await res.json(); if(data.success) { fetchUsers(); alert('Saved'); } else alert(data.message); } catch(e) { alert('Error'); } } setApiLoading(false); setUserModalOpen(false); 
  };
          ...
          {view === 'settings' && (
             <div className="max-w-4xl mx-auto">
                 ...
                 <div className="flex space-x-4 mb-6 border-b border-gray-200 overflow-x-auto">
                     ...
                     <button onClick={() => setSettingsTab('account')} className={`pb-3 px-2 font-medium transition-colors whitespace-nowrap ${settingsTab === 'account' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500'}`}>Account & Verification</button>
                     ...
                     <Card>
                         ...
                         <div className="space-y-4">
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium mb-1">Qiospay API Key</label>
                                    <div className="relative">
                                        <input type="password" className="w-full border p-2 rounded pr-10" value={config.qiospayApiKey || ''} onChange={e => setConfig({...config, qiospayApiKey: e.target.value})} disabled={currentUser.role === 'user'} />
                 ...
                 {/* TAB 2: Account Profile & Verification */}
                 {settingsTab === 'account' && (
                                     ...
                                     <div className="space-y-4">
                                         <div>
                                             ...
                                             <div className="relative">
                                                ...
                                                />
                                                <button type="button" onClick={() => setShowCurrentPass(!showCurrentPass)} className="absolute right-3 top-2 text-gray-400 hover:text-gray-600">
                                         ...
                                         <div>
                                             ...
                                             <div className="relative">
                                                ...
                                                />
                                                <button type="button" onClick={() => setShowNewPass(!showNewPass)} className="absolute right-3 top-2 text-gray-400 hover:text-gray-600">
                                         ...
                                         <div>
                                             ...
                                             <div className="relative">
                                                ...
                                                />
                                                <button type="button" onClick={() => setShowConfirmNewPass(!showConfirmNewPass)} className="absolute right-3 top-2 text-gray-400 hover:text-gray-600">
                                                    {showConfirmNewPass ? <EyeOff size={18} /> : <Eye size={18} />}
                                                </button>
                                             </div>
                                             {accountForm.newPassword && accountForm.newPassword !== accountForm.confirmNewPassword && (
                 ...
                 {settingsTab === 'smtp' && (
                     <Card>
                         ...
                         <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-6 flex gap-3">
                             ...
                             <div className="text-sm text-blue-800"><strong>Why configure SMTP?</strong><br/>Send automated payment receipts to your customers.</div>
                         </div>
                         <div className="mb-6 p-4 border border-indigo-100 rounded-lg bg-indigo-50/50">
                             <div className="flex items-center gap-2">
                                 <input type="checkbox" id="useSystemSmtp" className="h-5 w-5 text-indigo-600 rounded" checked={config.smtp?.useSystemSmtp || false} onChange={e => setConfig({...config, smtp: {...config.smtp!, useSystemSmtp: e.target.checked}})} />
                                 <div><label htmlFor="useSystemSmtp" className="text-base font-medium text-gray-900 cursor-pointer">Use System Default SMTP</label><p className="text-sm text-gray-500">If enabled, the system will use the Superadmin's email server to send your notifications.</p></div>
                             </div>
                         </div>
                         <div className={`space-y-4 transition-opacity duration-200 ${config.smtp?.useSystemSmtp ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                 <div><label className="block text-sm font-medium mb-1">SMTP Host</label><input type="text" className="w-full border p-2 rounded" value={config.smtp?.host || ''} onChange={e => setConfig({...config, smtp: {...config.smtp!, host: e.target.value}})} /></div>
                                 <div><label className="block text-sm font-medium mb-1">SMTP Port</label><input type="text" className="w-full border p-2 rounded" value={config.smtp?.port || ''} onChange={e => setConfig({...config, smtp: {...config.smtp!, port: e.target.value}})} /></div>
                                 <div><label className="block text-sm font-medium mb-1">Username</label><input type="text" className="w-full border p-2 rounded" value={config.smtp?.user || ''} onChange={e => setConfig({...config, smtp: {...config.smtp!, user: e.target.value}})} /></div>
                         ...
                         <div className="mt-6 pt-6 border-t border-gray-100">
                            ...
                            <div className="flex items-center gap-2 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                 <input type="checkbox" id="reqVerif" className="h-4 w-4 text-indigo-600 rounded" checked={config.smtp?.requireEmailVerification || false} onChange={e => setConfig({...config, smtp: {...config.smtp!, requireEmailVerification: e.target.checked}})} />
                                 <div>
                                     <label htmlFor="reqVerif" className="text-sm font-bold text-gray-700">Require Email Verification for New Users</label>
                                     <p className="text-xs text-gray-500">If checked, new users created under you must verify their email before using the dashboard.</p>
                                 </div>
                             </div>
                         </div>
