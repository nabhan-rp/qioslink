...
// --- CONFIGURATION ---
const APP_VERSION = "3.8.0 (SMTP System Fallback)";
    ...
    <div className="min-h-screen bg-gray-50 flex overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10">
          {/* Other views (Dashboard, Terminal, Links, History, Users, Integration) are hidden for brevity but exist in logic above */}
          {view !== 'settings' && view !== 'dashboard' && view !== 'terminal' && view !== 'links' && view !== 'history' && view !== 'users' && view !== 'integration' && null}
          ...
          {/* SETTINGS VIEW (UPDATED: With System SMTP Toggle) */}
          {view === 'settings' && (
                     ...
                     <Card>
                         ...
                         <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 mb-6 flex gap-3">
                             ...
                             <div className="text-sm text-blue-800">
                                 ...
                                 Send automated payment receipts to your customers.
                             </div>
                         </div>

                         {/* SYSTEM FALLBACK TOGGLE */}
                         <div className="mb-6 p-4 border border-indigo-100 rounded-lg bg-indigo-50/50">
                             <div className="flex items-center gap-2">
                                 <input 
                                     ...
                                     checked={config.smtp?.useSystemSmtp || false} 
                                     onChange={e => setConfig({...config, smtp: {...config.smtp!, useSystemSmtp: e.target.checked}})} 
                                 />
                                 <div>
                                     <label htmlFor="useSystemSmtp" className="text-base font-medium text-gray-900 cursor-pointer">Use System Default SMTP</label>
                                     <p className="text-sm text-gray-500">
                                         If enabled, the system will use the Superadmin's email server to send your notifications. You don't need to configure anything below.
                                     </p>
                                 </div>
                             </div>
                         </div>
                         ...
                         {/* NOTIFICATION TOGGLE */}
                         <div className="flex items-center gap-2 p-4 bg-gray-50 rounded-lg border border-gray-200 mt-6">
                             <input 
                                 type="checkbox" 
                                 id="enableNotif" 
                                 className="h-4 w-4 text-indigo-600 rounded" 
                                 checked={config.smtp?.enableNotifications || false} 
                                 onChange={e => setConfig({...config, smtp: {...config.smtp!, enableNotifications: e.target.checked}})} 
                             />
                             <label htmlFor="enableNotif" className="text-sm font-medium text-gray-700">Enable email notifications for new payments.</label>
                         </div>
                         ...
                         <div className="flex gap-4 pt-4 border-t border-gray-100 mt-4">
                             ...
                             <button onClick={handleTestEmail} disabled={apiLoading || config.smtp?.useSystemSmtp} className="bg-gray-100 text-gray-700 px-6 py-2 rounded-lg font-bold hover:bg-gray-200 flex items-center gap-2 border border-gray-200" title={config.smtp?.useSystemSmtp ? "Save config to test system SMTP" : ""}>{apiLoading ? <Loader2 className="animate-spin" size={18}/> : <Send size={18}/>} Test Email</button>
                         </div>
                     </Card>
                 )}
             </div>
