...
// Fix: Cast the env object to any to avoid "property does not exist on type {}" errors when accessing custom environment variables.
...
export default function App() {
  ...
  const handlePublicCheck = async (silent = false) => {
      ...
      if (IS_DEMO_MODE) {
          if (!silent) alert("Check Status Demo (No connection to backend)");
          return;
      }
          ...
          if (token) {
               ...
               if (data.success) {
                   ...
                   if (data.data.status === 'paid' && !silent) {
                       alert("Success! 1 Payment(s) Confirmed!");
  ...
  const handleForgotPasswordRequest = async (e: React.FormEvent) => {
      e.preventDefault();
      setForgotError('');
      if (!forgotEmail) return setForgotError("Email required");
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
      e.preventDefault();
      setForgotError('');
      if (forgotNewPass !== forgotConfirmPass) return setForgotError("Passwords do not match");
      
      setApiLoading(true);
      
      if (IS_DEMO_MODE) {
          if (forgotOtp === '123456') {
              alert("Password Reset Successful (Demo). Please login.");
              ...
              setForgotError("Invalid OTP (Demo: 123456)");
          }
          setApiLoading(false);
      } else {
          try {
          ...
          return (
              <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                  <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative">
                      <button onClick={() => { setShowForgotPassword(false); setForgotStep(1); setForgotError(''); }} className="absolute top-4 right-4 p-2 bg-gray-100 hover:bg-gray-200 rounded-full transition-colors">
                          <X size={20} className="text-gray-600" />
                      </button>
                      <div className="bg-indigo-600 p-8 text-center">
                          <h1 className="text-2xl font-bold text-white">Reset Password</h1>
                          <p className="text-indigo-100 text-sm mt-1">
                              {forgotStep === 1 ? "Enter your email to receive an OTP code." : "Enter OTP and new password."}
                          </p>
                      </div>
                      ...
                      <div className="p-8">
                          {forgotStep === 1 ? (
                              <form onSubmit={handleForgotPasswordRequest} className="space-y-4">
                                  ...
                                  {forgotError && <div className="p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center"><AlertCircle size={16} className="mr-2"/>{forgotError}</div>}
                                  <button type="submit" disabled={apiLoading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg font-bold shadow-lg shadow-indigo-500/30 transition-all">
                                      {apiLoading ? <Loader2 className="animate-spin mx-auto"/> : 'Send OTP Code'}
                                  </button>
                              </form>
                          ) : (
                              <form onSubmit={handleResetPassword} className="space-y-4">
                                  <div>
                                      ...
                                      <input 
                                          ...
                                          required 
                                          maxLength={6}
                                          className="w-full px-4 py-3 border border-gray-200 rounded-lg text-center tracking-widest font-bold text-xl" 
                                          value={forgotOtp} 
                                  ...
                                  {forgotError && <div className="p-3 bg-red-50 text-red-600 text-sm rounded-lg flex items-center"><AlertCircle size={16} className="mr-2"/>{forgotError}</div>}
                                  <button type="submit" disabled={apiLoading} className="w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg font-bold shadow-lg shadow-green-500/30 transition-all">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-72 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:relative lg:translate-x-0`}>
        <div className="h-full flex flex-col">
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
             ...
             {['superadmin', 'merchant'].includes(currentUser.role) && <SidebarItem active={view === 'integration'} icon={<Code2 size={20} />} label="Integration" onClick={() => { setView('integration'); if(window.innerWidth < 1024) setSidebarOpen(false); }} />}
          ...
          {view === 'dashboard' && (
              <div className="space-y-6">
                {currentUser.role === 'user' && (
                    <Card className="bg-gradient-to-r from-indigo-50 to-blue-50 border-indigo-100 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-indigo-600 rounded-full flex items-center justify-center text-white shrink-0"><ShoppingBag size={24}/></div>
                            <div><h3 className="text-lg font-bold text-gray-800">Become a Merchant</h3><p className="text-sm text-gray-500">Upgrade your account to accept payments and manage your own QRIS Nobu.</p></div>
                        </div>
                        <button 
                            onClick={() => window.open(`https://wa.me/628123456789?text=Halo%20Admin,%20saya%20mau%20upgrade%20jadi%20Merchant%20QiosLink.%20Username:%20${currentUser.username}`, '_blank')}
